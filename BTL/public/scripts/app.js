...
if ($(document).ajaxError((function(e, t, n, o) {
        var i = "",
            a = t.responseJSON;
        if (a) {
            if ("object" === _typeof(a.errors))
                for (var s in a.errors) a.errors.hasOwnProperty(s) && a.errors[s].forEach((function(e) {
                    i += e + "\n"
                }));
            else i = a.message;
            alert(i)
        } else console.log(t.statusText)
    })), $("time.timeago").timeago(), token = $('meta[name="csrf-token"]').attr("content"), $("#mainpart").css({
...
if ($(".daily-recent_views .slider").length && tns(Object.assign({
    ...
    }, sliderOptions)), $(".js-finished-series .slider").length && tns(Object.assign({
    ...
    }, sliderOptions)), $(".mobile-toggle header").click((function() {
        ...
        e.stopPropagation(), $(this).find(".toggle-icon").find("i").toggleClass("fa-toggle-off fa-toggle-on"), Cookies.get("night_mode") ? (Cookies.remove("night_mode"), $("#night-mode-css").remove(), $("html").removeClass("dark")) : (Cookies.set("night_mode", !0, {
            expires: 365
        }), $("html").addClass("dark"), window.location.reload())
        ...
        Alpine.store("toast", {
            ...
            show: function(e) {
                var t = this;
                this.on = !0, this.message = e, this.resetTime(), this.timeout = setTimeout((function() {
                    return t.hide()
                }), 3e3)
            },
            hide: function() {
    ...
    })), "undefined" != typeof chapter_id && chapter_id > 0 && (bookmark_data = {
        _token: token,
    ...
    var readingSeries = JSON.parse(localStorage.getItem("reading_series")) || [];
    for (i = 0; i < readingSeries.length; i++)
        if (readingSeries[i].series_id == series_id) {
            readingSeries.splice(i, 1);
            break
        }
    readingSeries.unshift(readingObject), readingSeries.length > 100 && readingSeries.pop(), localStorage.setItem("reading_series", JSON.stringify(readingSeries))
}

function seeMoreButton(e) {
    (e.find("img[alt]").length || e[0].scrollHeight >= 90) && e.next(".comment_see_more").removeClass("none")
}
...
if ($(".ln-comment-body").on("click", "span.span-pin", (function() {
        $.post("/action/comment/pin_comment", {
            _token: token,
        ...
        $.post("/action/comment/like-unlike", {
            ...
            "success" == t.status ? (console.log(e), t.liked ? (e.addClass("liked"), e.find("span.likecount").text(t.like_count)) : (e.removeClass("liked"), e.find("span.likecount").text(t.like_count))) : alert(t.message)
        }))
    })), seeMoreButtons(), $(".ln-comment-body").on("click", ".comment_see_more", (function(e) {
        clickSeeMore($(this))
        ...
        $.post("/action/comment/new", {
            ...
            if ("success" == e.status && "" != e.html) {
                ...
                }), $("#ln-comment-submit").after($('<div class="ln-comment-group">' + e.html + "</div>").fadeIn(700)), tinymce.activeEditor.setContent(""), seeMoreButton($("#ln-comment-" + e.comment_id).find(".ln-comment-content"))
        ...
        if ($("#ln-comment-" + t).next().find("textarea.comment_reply").length) $(".reply-form").remove();
            ...
            $(".reply-form").remove();
            var o = t != n ? "@" + $("#ln-comment-" + t + " a.ln-username").text().trim() + ":&nbsp;" : "";
        ...
        return $.post("/comment/ajax_paging", {
            ...
            if ("success" == e.status && "" != e.html) {
                ...
                seeMoreButtons(), $("html,body").animate({
                    scrollTop: n.offset().top - $("body").offset().top + n.scrollTop()
                })
            } else alert(e.message);
        ...
        var t = $(this);
        t.next().show(), $.post("/comment/fetch_reply", {
            _token: token,
            parent_id: t.data("parent"),
            offset: t.parent().find(".ln-comment-item").length,
            after: t.parent().find(".ln-comment-item").last().data("comment")
        }, (function(e) {
            "success" == e.status && "" != e.html ? (t.next().hide(), e.remaining > 0 ? t.text(e.fetchReplyText) : t.hide(), t.before(e.html), seeMoreButtons()) : "error" == e.status && alert(e.message)
        }), "json")
    ...
    }))), "1" == $('meta[name="logged-in"]').attr("content") && !$("main.reading-page").length) {
    var handleNotiData = function(e) {
            e.notification_count > 0 ? ($("#noti-icon").find(".noti-unread").remove(), $("#noti-icon .icon-wrapper").append('<span class="noti-unread">' + e.notification_count + "</span>"), $("#noti-icon #noti-sidebar").find("#noti-content").prepend(e.html)) : $("#noti-icon").find(".noti-unread").remove(), e.seriesunread_count > 0 ? ($("#series-unread-icon").find(".noti-unread").remove(), $("#series-unread-icon .icon-wrapper").append('<span class="noti-unread">' + e.seriesunread_count + "</span>")) : $("#series-unread-icon").find(".noti-unread").remove(), e.pmunread_count > 0 ? ($(".at-user_avatar").addClass("icon-notify"), $(".at-user_list").addClass("icon-notify")) : ($(".at-user_avatar").removeClass("icon-notify"), $(".at-user_list").removeClass("icon-notify")), unreadCount = parseInt(e.total), setDocumentTitle(unreadCount > 0 ? "(" + unreadCount + ") " + pageTitle : pageTitle)
        },
    ...
    $("span.noti-unread").each((function() {
        ...
        })), unreadCount > 0 && (document.title = "(" + unreadCount + ") " + document.title), $("#noti-icon").on("click", (function(e) {
            e.stopPropagation();
            ...
            "" != t.text().trim() && $.post("/action/notification/clearunread", {
                ...
                "success" == e.status ? (t.remove(), unreadCount = 0, $("span.noti-unread").each((function() {
                    unreadCount += parseInt($(this).text())
        ...
        function e() {
            setTimeout((function() {
                var t = $(".noti-item time.timeago").first().attr("title") || "";
                ((new Date).getTime() / 1e3 | 0) - (localStorage.getItem("ln_refresh_time") || 0) < refreshTime ? e() : (localStorage.setItem("ln_refresh_time", (new Date).getTime() / 1e3 | 0), $.post("/action/notification/fresh", {
                    last: t
                }, (function(t) {
                    "success" == t.status && (handleNotiData(t), localStorage.setItem("ln_crosstab", JSON.stringify(t))), e()
                }), "json"))
            }), 1e3 * refreshTime)
        }(), $(window).on("storage", (function(e) {
            "ln_crosstab" == e.originalEvent.key && handleNotiData(JSON.parse(localStorage.getItem("ln_crosstab")))
        }))
}
if ($(".spoiler_toggle").on("click", (function(e) {
        e.preventDefault();
        var t, n = $(this);
    ...
    $(".search-advance_toggle").on("click", (function() {
        ...
        e.preventDefault(), $(".gerne-icon i").each((function(e, t) {
            var n = $(this),
                o = n.parents("label").data("genre-id");
            n.hasClass("fa-check") ? selectGenres.add(o) : n.hasClass("fa-times") && rejectGenres.add(o)
        }));
        var t = "?selectgenres=" + _toConsumableArray(selectGenres).join(",") + "&rejectgenres=" + _toConsumableArray(rejectGenres).join(",");
        t += "&" + $(this).serialize(), window.location.href = window.location.href.split("?")[0] + t
    }))
}
$(".container").width() <= 768 && $(".gradual-mobile").find("main").hide(), $(".gradual-mobile header").on("click", (function() {
    ...
    $.post("/action/series/collect", {
        ...
        var t = $("#collect");
        if ("success" == e.status) {
            var n = e.collected ? "Bạn đã theo dõi truyện." : "Bạn đã ngừng theo dõi truyện.";
            t.toggleClass("follow followed"), t.find("i").toggleClass("far fas"), Alpine ? Alpine.store("toast").show(n) : alert(n)
        } else $(".summary-content").css({
            maxHeight: "150px",
            overflow: "hidden"
        }), _this.html('<i class="fa fa-angle-double-down" aria-hidden="true"></i> Xem thêm')
    }), "json")
})), $(".feature-section .summary-content").length && $(".feature-section .summary-content")[0].scrollHeight >= 100 && $(".feature-section .summary-more").removeClass("none"), $(".mobile-more").click((function(e) {
